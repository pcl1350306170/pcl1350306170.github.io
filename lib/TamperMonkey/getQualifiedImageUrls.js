...
(function () {
    ...
    class PageCrawler {
         ...
         */
        init() {
            if (this.currentUrl.includes('v33.d58gx9.cc/pw/thread')) {
                this.initThreadPage(); // 列表页初始化
            } else if (this.currentUrl.includes('/pw/read.php')) {
        ...
        openList(type) {
            ...
            const validATags = Array.from(table.getElementsByTagName('a'))
                .filter(link => {
                    ...
                    const isHrefValid = href && href.includes('read.php') &&
            ...
            const linksData = validATags
                .map(link => {
                    const href = link.getAttribute('href');
                    // 根据type返回不同格式的链接
                    return type === 1 ? href : `https://v33.d58gx9.cc/pw/${href}`;
                })
        ...
        saveLinks(links, validATags) {
            ...
            const hrefToTextMap = validATags.reduce((map, link) => {
                const href = link.getAttribute('href');
                const fullHref = `https://v33.d58gx9.cc/pw/${href}`;
