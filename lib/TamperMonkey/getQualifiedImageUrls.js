...
// @author       You
// @match        https://191916.ca606c9b.cc/pw/*
// @match        https://191916.ca606c9b.cc/pw/*
...
/**
 * 列表传参【自动翻页和保存类型】： &pageFrom=novels&dataType=V33-IMG-露
 * https://191916.ca606c9b.cc/pw/thread111.php?fid=210&page=3&pageFrom=novels
 * 
 * 详情页传参：pageFrom=image&source=V33-IMG-AI
 * 脚本配置即可
 */
(function () {
    'use strict';
    ...
    class PageCrawler {
        constructor() {
            ...
            this.serveApi = 'http://192.168.3.227:28019/crawl-api/v1/webCrawlData/add';
            this.serveApiHost = 'http://192.168.3.227:28019/';
        ...
        openList(type) {
            ...
            const linksData = validATags
                .map(link => {
                    const href = link.getAttribute('href');
                    // 根据type返回不同格式的链接
                    return type === 1 ? href : `https://191916.ca606c9b.cc/pw/${href}`;
                })
        ...
        saveLinks(links, validATags) {
            // 1. 从URL参数获取dataType值，默认为"V33-IMG-AI"
            const urlParams = new URLSearchParams(window.location.search);
            const dataTypeValue = urlParams.get('dataType') || 'V33-IMG-AI';

            // 2. 建立「链接-文本」映射表，方便快速匹配
            const hrefToTextMap = validATags.reduce((map, link) => {
                const href = link.getAttribute('href');
                const fullHref = `https://191916.ca606c9b.cc/pw/${href}`;
            ...
            console.log('提交数据示例：', postData.slice(0, 2)); // 打印前两条数据用于调试

            // 5. 使用 GM_xmlhttpRequest 替代 $.ajax，解决跨域问题
            GM_xmlhttpRequest({
                method: 'POST',
                ...
                onload: (response) => {
                    ...
                    if (response.status >= 200 && response.status < 300) {
                        ...
                        console.error('链接保存失败，状态码：', response.status);
                ...
                onerror: (error) => {
                    ...
                    console.log('请检查：');
                    console.log('1. 服务器是否启动：', this.serveApiHost);
                    console.log('2. 接口地址是否正确：', `${this.serveApiHost}api/v1/general-data/batch-save`);
         ...
         */
        saveImgs() {
            // 1. 从URL参数获取source值，默认为"v33图库-AI"
            const urlParams = new URLSearchParams(window.location.search);
            const sourceValue = urlParams.get('source') || 'v33图库-AI';
            const generalDataId = urlParams.get('generalDataId') || '';
            ...
            GM_xmlhttpRequest({
                ...
                onload: (response) => {
                    ...
                    if (response.status >= 200 && response.status < 300) {
                        try {
                            const result = JSON.parse(response.responseText);
                            console.log('图片信息保存成功', result);
                            setTimeout(() => window.close(), 2000); // 2秒后关闭页面
                        } catch (e) {
                            console.error('响应解析失败', e);
                            console.log('原始响应：', response.responseText);
                            setTimeout(() => window.close(), 2000); // 即使解析失败也关闭
                        }
                    } else {
                        console.error('图片信息保存失败，状态码：', response.status);
                        console.error('错误信息：', response.statusText);
                        console.error('响应内容：', response.responseText);
                        setTimeout(() => window.close(), 3000); // 失败后延迟关闭，方便查看错误
                    }
                },
                onerror: (error) => {
                    console.error('请求发送失败', error);
                    console.log('请检查：');
                    console.log('1. 服务器是否启动：', this.serveApi);
                    console.log('2. 网络连接是否正常');
                    setTimeout(() => window.close(), 3000); // 失败后延迟关闭
                }
            });
        }
    }
